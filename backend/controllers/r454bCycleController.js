...
/**
 * Calculate R454B 4-point cycle for plotting via CoolProp mixture calculation.
 * POST /api/r454b/cycle/plot
...
exports.calculateCyclePlottingPoints = async (req, res) => {
  try {
    ...
    console.error('[R454B CoolProp] cycle calc error:', error.message);
...
exports.calculateBatchPointProps = async (req, res) => {
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      proc.stdin.write(JSON.stringify({ refrigerant: 'R454B', points }));
      proc.stdin.end();
    });

    res.json(result);
  } catch (error) {
    console.error('[R454B CoolProp] batch props error:', error.message);
    res.status(500).json({ status: 'error', message: error.message });
  }
};
