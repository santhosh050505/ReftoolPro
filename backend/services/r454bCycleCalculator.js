const r454bSuperheatService = require('./r454bSuperheatService');
const r454bSaturationService = require('./r454bSaturationService');

/**
 * Calculate R454B 4-Point Thermodynamic Cycle for Plotting
 * STRICT LOOKUP MODE (No Interpolation) - Matches R513A logic
 * 
 ...
 */
function calculateR454BCyclePlottingPoints(sct, set, dischargeTempRef, suctionTempRef, liquidTempRef) {
  try {
    ...
    const satPressSCT = r454bSaturationService.getSaturationPressure(sct);
    const satPressSET = r454bSaturationService.getSaturationPressure(set);
    ...
    let p1_lookup = r454bSuperheatService.getSuperheatPropertiesNearest(P_sat_SCT, dischargeTempRef);
    ...
    if (p1_lookup) {
      ...
      const p1_sat = r454bSaturationService.getVaporProperties(sct);
      if (p1_sat) {
        p1_props = {
    ...
    const p2_lookup = r454bSaturationService.getLiquidProperties(liquidTemp);
    ...
    const p3_lookup = r454bSaturationService.getAverageProperties(set);
    ...
    let p4_lookup = r454bSuperheatService.getSuperheatPropertiesNearest(P_sat_SET, suctionTempRef);
    ...
    if (p4_lookup) {
      ...
      const p4_sat = r454bSaturationService.getVaporProperties(set);
      if (p4_sat) {
        p4_props = {
    ...
    console.error('Error calculating R454B cycle:', error);
    return {
      status: 'error',
...
module.exports = {
  calculateR454BCyclePlottingPoints,
  validateCycleInputs
};
